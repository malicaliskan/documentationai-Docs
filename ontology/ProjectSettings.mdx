...
Per-environment base URLs for the frontend application. These URLs are used to derive callback URLs (e.g. SOCIAL_LOGIN_REDIRECT_URL = frontendUrl + /auth/callback). If not set, defaults are derived from the project codename: dev=http://localhost:5173, test=https://{codename}.prw.mindbricks.com, stage=https://{codename}-stage.mindbricks.co, prod=https://{codename}.mindbricks.co.
