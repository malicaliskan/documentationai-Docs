---
title: "AI Agent"
description: "Defines a standalone AI agent at the service level. Each agent encapsulates a model configuration, system prompt, input/output pipeline, optional tool access, and conversation management. Agents suppo"
---
...
interface AIAgent = {
  ...
  kafkaControllers : AgentKafkaController[];
}
```
...
| kafkaControllers      | Kafka-triggered controllers for the agent. Each controller listens on a Kafka topic and maps the incoming message payload to the agent's input format before executing the agent. Multiple controllers can be defined for different topics with different data mappings. |
...
interface AgentToolSettings = {
  ...
  integrationTools : AgentIntegrationToolConfig[];
}
```
...
| integrationTools | Integration providers to expose as agent tools. Each entry maps a project integration to tool definitions. Explicit method names create individual tools with full JSON Schema parameters. Wildcard '\*' creates a single dispatcher tool with a getIntegrationMethodDoc helper for looking up exact signatures on demand. |
...
### AgentIntegrationToolConfig

**MPO Version:** 1.3.0

Configures an integration provider as a tool source for the agent. Explicit method names (comma-separated) create individual tools with full JSON Schema parameters. Wildcard '\*' creates a single dispatcher tool per integration plus a shared getIntegrationMethodDoc helper for looking up exact method signatures on demand.

```typescript
interface AgentIntegrationToolConfig = {
...
| Field           | Description                                                                                                                                                                                             |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| integrationName | The provider key of the integration (e.g., 'stripe', 'amazonS3'). Must match a provider added to the project's integrations.                                                                            |
| methods         | Comma-separated list of method names to expose as individual tools, or '_' to register a single dispatcher tool with a documentation lookup helper. Examples: 'createCustomer,getPaymentIntent' or '_'. |
...
## AgentKafkaController

**MPO Version:** 1.3.0

Defines a Kafka-triggered controller for the AI agent. When enabled, a KafkaListener is created that subscribes to the specified topic. Incoming messages are mapped to the agent's request format using the dataMapping expression, then the agent is executed. Optionally, the agent's response can be published to a response topic. Multiple controllers can be defined per agent for different topics with different data mappings.

```typescript
interface AgentKafkaController = {
  name : String;
  topic : String;
  groupId : String;
  dataMapping : MScript;
...
| Field       | Description                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| name        | Unique identifier for this Kafka controller within the agent. Used in generated code and log correlation. Must be camelCase.                                                                                                                                                                                                                                                                                                                              |
| topic       | Kafka topic to subscribe to. When a message arrives on this topic, the agent is executed with the mapped data. Can use topic aliases (resolved at runtime via resolveTopicAlias).                                                                                                                                                                                                                                                                         |
| groupId     | Optional Kafka consumer group ID. If not specified, a default group ID is generated from the service codename and controller name. Use a custom group ID for shared consumption across service instances.                                                                                                                                                                                                                                                 |
| dataMapping | MScript expression that transforms the incoming Kafka message into the agent's request body. Has access to 'message' (parsed JSON payload), 'session' (extracted session object), 'topic' (topic name), and 'headers' (Kafka message headers). Should return an object matching the agent's expected input shape (e.g., \{ message: message.text, context: message.metadata \}). If null, the raw Kafka message payload is forwarded as the request body. |
| enabled     | Set to true to activate this Kafka controller. When false, the listener is not created during service startup. Default is true.                                                                                                                                                                                                                                                                                                                           |
