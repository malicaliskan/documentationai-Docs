...
A property opts into static-join sourcing by populating its `staticJoin` block. Block presence is the activation — the runtime drops the property from input parameters (callers cannot supply it) and fills it from the configured join at create / update time.
...
- **`staticJoin.contextParameterName`** (`String`, optional)

  - When set, the joined value is also exposed on the API context as `this.<contextParameterName>` so per-API `dataClauseItems` MScript can compose with it.

These fields tell Mindbricks *how to navigate from the current record to the source object* and *which value to copy*.

---

## 4. Execution Lifecycle & dataClauseItems Integration

### 4.1 Order of Operations in a Create/Update API

For a given property:

1. **Static join runs first** when `staticJoin` is populated.
   ...
   - Mindbricks:
     ...
     - Writes that value into the property's field, and exposes the full fetched record under a join-named context key (e.g. `this.user`). When `contextParameterName` is set, the joined value is also accessible as `this.<contextParameterName>`.

2. **Per-API `dataClauseItems[]` entries run afterwards.**

   - These MScript expressions can access:

     - The fetched object via `this.<jointSource>` (e.g. `this.user.tenantId`).

     - The joined value via `this.<contextParameterName>` when set.

This layered design lets you chain logic:

- Use static join to fetch `clientId` from user.

- Use a `dataClauseItems` entry to compute something else using that `clientId`.
...
2. Open the **Static Join** section and fill in:
...
Populate the property's `staticJoin` block (flat — no `configuration` wrapper) with valid `jointSource`, `jointSourceKey`, `jointForeignKey`, and `jointRead` (or let some defaults apply). Block presence is the activation; no other flag is needed.
