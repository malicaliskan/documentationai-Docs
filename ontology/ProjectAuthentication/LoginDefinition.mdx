...
description: "Defines the core login architecture of the project, including the user and tenant structures that underpin authentication and access. These settings enable Mindbricks to generate a built-in 'auth' ser"
---
...
| superAdminIdentifier               | The login identifier of the super admin user. When primaryLoginIdentifier is 'email' or 'emailOrMobile', this should be an email address. When primaryLoginIdentifier is 'mobile', this should be a mobile number in E.164 format (e.g. +905551234567). This user has full permissions across the project and is not tenant-scoped.                                                                            |
...
| dualIdentifierRegistration         | Only applies when primaryLoginIdentifier is 'emailOrMobile'. Controls whether users must provide both email and mobile during registration, or just one of them. When 'atLeastOne', users can register with either identifier. When 'both', users must provide both.                                                                                                                                           |
...
| mobileVerificationRequiredForLogin | Indicates whether users must verify their mobile number before logging in. This applies only when the mobile field is present in the user data model (i.e. secondaryIdentifierPresence is not 'none' when email is primary, or always when mobile is primary or emailOrMobile).                                                                                                                                |
...
Controls registration behavior when primaryLoginIdentifier is 'emailOrMobile'. Ignored for other primaryLoginIdentifier values.
