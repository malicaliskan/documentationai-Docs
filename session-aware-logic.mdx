...
- Per-API customData entries (`apiOptions.dataClauseSettings.customData[].value`) — including session-derived values like `"this.session.preferredLocale"` for properties whose `basicSettings.source` is `customData`
...
- Per-API customData entries (`apiOptions.dataClauseSettings.customData[].value`) on properties with `source: 'customData'` can compute values based on session: e.g., initial timezone, language code, default currency. Each create/update API declares its own MScript expression — e.g., `{ "name": "preferredCurrency", "value": "this.session.tenant.defaultCurrency ?? 'USD'" }`.
