...
For array field updates in `UpdateCrudAction`/db utilities, use array mutation operators (`$add`, `$rem`, `$addIf`) as documented in [Array Mutation Syntax Guide](./array-mutation-syntax-guide).
...
✅ **Do**: Use `onActionError: "completeApi"` for non-critical bulk operations. The error is stored as `this.<actionName>Error` so follow-up actions can check and handle it.
