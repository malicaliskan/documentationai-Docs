...
A relation is declared on a **property** of a DataObject (the "foreign key" field) by setting `basicSettings.hasRelation: true` and providing the flat `PropertyRelation` block at `relationSettings`. The activation flag lives on `basicSettings` (parallel to `source`); the `relationSettings` block has no `configuration` wrapper.
...
(`hasRelation` lives on `PropertyBasicSettings` — populate `relationSettings` only when it is `true`.)

### 2.1 What This Means

When you set `basicSettings.hasRelation: true` on a property, you're saying:
...
```json
{
  "basicSettings": { "name": "userId", "type": "ID", "hasRelation": true },
  "relationSettings": {
...
```json
{
  "basicSettings": { "name": "ownerId", "type": "ID", "hasRelation": true },
  "relationSettings": {
...
```json
{
  "basicSettings": { "name": "projectId", "type": "ID", "hasRelation": true },
  "relationSettings": {
...
{
  "basicSettings": { "name": "lastUpdateUserId", "type": "ID", "hasRelation": true },
  "relationSettings": {
