...
Enum configuration for the data property, applicable when the property type is set to Enum. Enum values are stored as strings in the database. When the property type is Enum, hasEnumOptions must be true and at least 2 enum options must be defined.
