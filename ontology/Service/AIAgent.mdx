...
| kafkaControllers      | Optional array of Kafka controllers that trigger agent execution from Kafka events. Each controller subscribes to a topic, maps the message payload to the agent's input format, and executes the agent automatically. |
