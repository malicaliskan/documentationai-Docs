...
- `dataClause` carries the write payload (custom data, forced literals via `customData`). Only `create` and `update` mutate fields; `delete` mutates only the soft-delete flag (configured via `deleteOptions`).
