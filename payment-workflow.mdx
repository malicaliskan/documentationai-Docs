...
`frontendAmountField` and `frontendCurrencyField`: Optional property references that tell frontend clients which fields should be displayed as payment amount and currency. This is useful when the backend amount/currency expression is complex and UI should show a stable business-facing value.
