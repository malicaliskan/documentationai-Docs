...
Invokes an AI model such as OpenAI or Anthropic to perform text generation, refinement, classification, or summarization. The prompt is composed from a template and context, and the result is returned to the workflow and stored in `this.<contextPropertyName>`. In SSE mode, tokens are streamed to the client automatically; in REST mode, the AI provider is called directly (non-streaming).
