...
Uses a declared action from the action store (either `AiCallAction` or `IteratorAction`).
...
**In REST context:** The action calls the AI provider's non-streaming API directly and returns the full response. Post-processing (JSON parsing, `isArray` handling) is applied to the result.
...
- REST mode: the AI provider is called directly (non-streaming) and the result is stored in `this.<contextPropertyName>`
