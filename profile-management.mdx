...
- **Preview:** `https://tickatme.prw.mindbricks.com/bucket`
...
This Tickatme application also includes a common public bucket that anyone can read, but only users with the `superAdmin`, `admin`, or `saasAdmin` roles can write (upload) to it.
