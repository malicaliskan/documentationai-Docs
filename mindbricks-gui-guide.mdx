...
| Property → Basic Settings       | `DataProperty.basicSettings` (`PropertyBasicSettings`) — carries `source`, `hasRelation`, `isFilterParameter`, `enumOptions` (when `type === 'Enum'`), and the rest of the per-property flags |
| Property → Index Settings       | `DataProperty.indexSettings` (`PropertyIndexSettings`)                                      |
| Property → Relation Settings    | `DataProperty.relationSettings` (`PropertyRelation`) — populated when `basicSettings.hasRelation: true`, flat (no `configuration` wrapper) |
| Property → Session Settings     | `DataProperty.sessionSettings` (`PropertySessionSettings`) — populated when `source: 'session'` |
| Property → Static Join          | `DataProperty.staticJoin` (`PropertyStaticJoin`) — populated when `source: 'joined'`         |
| (per-API custom data)           | When `source: 'customData'`, the value lives on each create/update BusinessApi's `apiOptions.dataClauseSettings.customData[]` (a `DataClauseSettings`) |
