...
`topPick` is also part of `ListOptions` and can be combined with filters/pagination to cap ranked results before pagination is applied.
...
## 1.1 Ranked Cap with `topPick`

Use `topPick` to limit the ranked candidate set before pagination.  
This is especially useful with `listSortBy` for “latest/most relevant N” use cases.
...
With `pageRowCount = 25`, users can paginate only inside the first 1000 sorted rows.  
If `topPick` is `0` or `null`, no cap is applied.
