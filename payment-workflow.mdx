...
`orderIdProperty`: The property name that holds the order's unique identifier. Typically the ID field like `"id"`.

`amountProperty`: The property name that holds the order amount for payment. Typically the amount field like `"amount"` or `"totalAmount"`.

`currencyProperty`: The property name that holds the currency for the order. For example `"currency"`. This is optional if you use `currencyStaticValue` instead.

`currencyStaticValue`: The static value of the currency for the order (e.g., `"USD"`, `"EUR"`). If not set, the currency will be determined by the `currencyProperty`. Use this when all orders use the same currency.
...
`edgeFunctionPaymentDone`: Optional name of an edge function from the service library that is triggered only when payment is successfully completed from the callback flow. This hook is intended for critical post-payment operations that should run on successful completion (for example creating coins, issuing credits, or activating premium access). Since callback retries are possible in distributed systems, this function should be written as idempotent.

`orderItemRelation`: Optional relation definition between your order object and its order-item object. This is used by post-payment automation scenarios where records are emitted per item or per quantity.

`afterPaymentCreations`: Optional list of automatic creation rules that run after successful payment completion. Each rule targets a DataObject and supports emit modes:

- `perOrder`: one creation for the whole order
- `perOrderItem`: one creation per order item
- `perOrderItemQuantity`: multiple creations per item based on the quantity property in `orderItemRelation`
...
const response = await fetch(
  `${serviceUrl}/${generatedStartPaymentRoute}/${orderId}`,
  {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
...
`/{generated-start-payment-route}/:orderId`

By sending a request to this route using the service API address, you can execute this Business API. Route naming can vary by generated service conventions and version. Always verify the exact route from your service API documentation or generated assets reference, then use that concrete path. Parameters can be provided in multiple HTTP locations, including the URL path, URL query, request body, and request headers. Detailed information about these parameters is provided in the **Parameters** section.
...
**Select By**:
A list of fields that must be matched exactly as part of the WHERE clause. This is not a filter — it is a required selection rule. In single-record APIs (`get`, `update`, `delete`), it defines how a unique record is located. In `list` APIs, it scopes the results to only entries matching the given values. When a single non-ID field is specified (e.g., `['slug']`), the default route path uses that field instead of `/:objectId`, and the ID parameter is not auto-created.
...
You can use the following settings to change some behavior of this step.
`requestParameters`, `redisParameters`
...
To access the api use the generated REST controller path for your service (typically a `PATCH` route for `startOrderPayment` with `:orderId`).

```javascript
axios({
  method: "PATCH",
  url: `${serviceUrl}/${generatedStartPaymentRoute}/${orderId}`,
  data: {
