...
**Description:** Matches records where the specified field is present and non-null. Cross-backend: emits the native existence check on each platform (Mongo `$exists`, ES `exists` query, Sequelize `IS NOT NULL`). The boolean value is conventionally `true` and is ignored — for the inverse, use `$nexists`, not `$exists: false`.
...
**Description:** Matches records where the specified field is missing or null. Cross-backend: emits the native is-null check on each platform (Mongo `$exists: false`, ES `must_not.exists`, Sequelize `IS NULL`). The boolean value is conventionally `true` and is ignored.
...
* [MScript Syntax Guide](./mscript-syntax) — Complete MScript syntax reference