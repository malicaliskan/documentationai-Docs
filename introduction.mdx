...
3. **Compositional Logic**
   It resolves pattern interactions. For example, if a `RouteValidation` pattern depends on a `FunctionCall`, which in turn accesses a `ForeignFetch`, Genesis wires them together correctly. It ensures all logic flows match the semantic structure defined in the pattern graph.
