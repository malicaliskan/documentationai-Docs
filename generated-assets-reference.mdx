...
| `permissionName` | String | Yes | Reference to the named permission. Can be formatted as:<br>- `"groupName.permissionName"` (specific permission)<br>- `"permissionName"` (permission without group)<br>- `"groupName.*"` (all permissions in a group) |
...
#### User Data Object Topics

| Event Type | Topic Name | Triggered When | Message Structure |
|------------|------------|----------------|-------------------|
| `created` | `auth.user.created` | User is created | `{ user: {...userData} }` |
| `updated` | `auth.user.updated` | User is updated | `{ user: {...userData} }` |
| `deleted` | `auth.user.deleted` | User is deleted | `{ user: {...userData} }` |
