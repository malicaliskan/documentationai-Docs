...
**mobile** : The mobile number of the user being registered. Required when `primaryLoginIdentifier` is `'mobile'` or `'emailOrMobile'`. Also required when `primaryLoginIdentifier` is `'email'` and `secondaryIdentifierPresence` is `'required'`.
...
- Accepts either `username` or `email` fields (if both exist, `username` is prioritized). When `primaryLoginIdentifier` is `'mobile'`, pass the mobile number as `username`.
