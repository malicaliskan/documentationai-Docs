...
Mobile verification is available when the mobile field is present in the user data model (i.e., when `primaryLoginIdentifier` is `'mobile'` or `'emailOrMobile'`, or when `secondaryIdentifierPresence` is `'optional'` or `'required'` with email as primary). All mobile numbers must be in **E.164 format** (`+` followed by country code and subscriber number, e.g. `+905551234567`). The backend normalizes mobile numbers to this format automatically.
...
1. Call the mobile verification `start` route of the backend (described below) with the user's primary login identifier (email or mobile, depending on `primaryLoginIdentifier`). The backend will send a secret code to the user's mobile number. **If a real texting service is configured, the backend sends the SMS. During development, the backend also returns the secret code to the frontend in the&#x20;********`secretCode`********\*\*\*\*\*\*\*\* property.**
...
3. When the user submits the code, complete mobile verification using the `complete` route of the backend (described below) with the user's primary login identifier (email or mobile) and the secret code.
...
| Parameter | Type   | Required | Description                          |
| --------- | ------ | -------- | ------------------------------------ |
| `email` or `mobile` | String | Yes | User's primary login identifier (email or mobile depending on `primaryLoginIdentifier`) |
...
| Parameter    | Type   | Required | Description           |
| ------------ | ------ | -------- | --------------------- |
| `email` or `mobile` | String | Yes | User's primary login identifier |
...
Since there are two verification methods (by email or by mobile), when the reset password link is clicked, the frontend should determine which options are available based on the `primaryLoginIdentifier` and `secondaryIdentifierPresence` settings. If both email and mobile fields exist in the user model, the frontend should let the user choose which method to use. According to the user's selection, the frontend should start the related flow as explained below step by step.
...
> All mobile parameters in the password reset by mobile flow must be in E.164 format (e.g. `+905551234567`).

1. Call the password reset by mobile verification `start` route of the backend (described below) with the user's primary login identifier (email or mobile). The backend will send a secret code to the user's mobile number. **If a real texting service is configured, the backend sends the SMS. During development, the backend also returns the secret code to the frontend in the&#x20;********`secretCode`********\*\*\*\*\*\*\*\* property.**
...
5. When the user submits the code, complete mobile verification using the `complete` route of the backend (described below) with the user's primary login identifier (email or mobile) and the secret code.
...
| Parameter | Type   | Required | Description                                   |
| --------- | ------ | -------- | --------------------------------------------- |
| email or mobile | String | Yes | The primary login identifier of the user resetting the password |
...
| Parameter  | Type   | Required | Description                   |
| ---------- | ------ | -------- | ----------------------------- |
| email or mobile | String | Yes | The primary login identifier of the user |
