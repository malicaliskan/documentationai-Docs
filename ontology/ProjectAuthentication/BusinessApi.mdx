...
Per-API REST endpoint settings — primarily a custom routePath override. If `routePath` is omitted or set to `$default`, Mindbricks auto-generates the path from the API name, crudType, dataObjectName, and selector configuration. The default-route calculator always includes a path parameter on single-record APIs (`get`, `update`, `delete`) so each route addresses a specific record — see \_\_routePath.doc below for the full algorithm. The designer should set an explicit `routePath` when the auto-calculated route doesn't match their intent (e.g. nested resources, verb-style endpoints like `/getActiveBasket`, non-id selectors with multiple parts).
