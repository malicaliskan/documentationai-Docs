...
| email      | String | conditional | request.body?.email      |
| store      | Object | true     | request.body?.store      |
| fullname   | String | true     | request.body?.fullname   |
| mobile     | String | conditional | request.body?.mobile     |
...
**email** : The email/login identifier of the user being registered. Required when `primaryLoginIdentifier` is `'email'` or `'emailOrMobile'`. Also required when `primaryLoginIdentifier` is `'mobile'` and `secondaryIdentifierPresence` is `'required'`.
...
**mobile** : The mobile number of the user being registered, in E.164 format (e.g. `+905551234567`). The backend normalizes mobile numbers to this format automatically. Required when `primaryLoginIdentifier` is `'mobile'` or `'emailOrMobile'`. Also required when `primaryLoginIdentifier` is `'email'` and `secondaryIdentifierPresence` is `'required'`.
...
After successful registration and completing any required verifications, the user can log in. Please create a minimal, polished login page where the user can enter their login identifier and password. The login identifier depends on the `primaryLoginIdentifier` setting: it can be email (default), mobile number, or either (when set to `emailOrMobile`). Note that this page should respect the deployment (environment) selection option made in the home page to set the base URL. If the user reaches this page directly skipping home page, the default `production` deployment will be used.
...
| ---------- | ------ | -------- | ----------------------- |
| `username` | String | Yes      | `request.body.username` (email or mobile, depending on `primaryLoginIdentifier`) |
| `password` | String | Yes      | `request.body.password` |

#### Behavior

- The `username` field accepts the user's primary login identifier: email when `primaryLoginIdentifier` is `'email'`, mobile (E.164 format) when `'mobile'`, or either when `'emailOrMobile'`.
- **The lookup is always performed against the primary identifier column only.** When `primaryLoginIdentifier` is `'email'`, only the `email` column is queried. When `'mobile'`, only the `mobile` column is queried. When `'emailOrMobile'`, the system detects the format (email vs. phone number) and queries the matching column. Passing an email address when mobile is the primary identifier will **not** match any user, even if that user has an email stored in the database.
...
- The `email` body parameter is accepted as a legacy alias for `username`. If both `username` and `email` are present in the request body, `username` takes priority.

#### Example

```js
// When primaryLoginIdentifier is 'email' (default):
axios.post("/login", {
  username: "user@example.com",
  password: "securePassword",
});

// When primaryLoginIdentifier is 'mobile':
axios.post("/login", {
...
> **Note on bucket tokens:** The `userBucketToken` is for the **external bucket service** (general file uploads like documents and product images). **User avatars do not use the external bucket service** — they are uploaded via database buckets (dbBuckets) built into the auth service using the regular `accessToken`. Upload to `POST {authBaseUrl}/bucket/userAvatars/upload` and download publicly from `GET {authBaseUrl}/bucket/userAvatars/download/key/{accessKey}`.
