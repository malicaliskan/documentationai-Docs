...
- `DataMapItem.value` — every entry in a BusinessApi's `dataClauseItems[]` (the per-API write map for create / update APIs)

- `WhereClauseSettings.fullWhereClause`

- `ExtendedClause.condition`, `whereClause`
...
- `ExtendedClause.condition`

- `MembershipCheckAction.checkFor`

#### 2.4.2 Server-computed Field via `dataClauseItems`

A `totalPrice` property derived as `unitPrice * quantity`. The property
declaration carries only DB-shape concerns:

```json
{
  "basicSettings": { "name": "totalPrice", "type": "Double", "isRequired": true }
}
```

On every create/update API of the parent data object, add an entry to
`dataClauseItems[]` that supplies the value as MScript:

```json
{
  "dataClauseItems": [
    { "name": "totalPrice", "value": "this.unitPrice * this.quantity" }
  ]
}
```

Callers are not exposed to `totalPrice` in the API contract (no
matching `requestParameter`), and the value is computed server-side
on every write.

#### 2.4.3 Inline Arrow Function for Complex Logic

Sometimes you need a few steps — the `value` is still MScript:

```json
{ "name": "netAmount", "value": "(() => { const net = this.grossAmount - this.discountAmount; return Math.max(net, 0); })()" }
```
...
Then declare `shippingFee` as a system-managed property — DB-shape
only, no caller-facing surface:
...
…and on each create/update API of the parent data object, add a
`dataClauseItems` entry that calls the library:

```json
{
  "dataClauseItems": [
    { "name": "shippingFee", "value": "LIB.calculateShippingFee(this.order)" }
  ]
}
```

Best of both worlds: explicit per-API write declaration + custom calculation logic.
...
- **`get${ModelName}ListByMQuery(mQuery)`** — Retrieves multiple records matching an MScript Query
  - **Parameters:** `mQuery` (Object) — MScript Query object (database-agnostic format, see [MScript Query Operators Reference](./mscript-query-reference))
  - **Returns:** Array — Array of matching records (empty array if none found)
  - **Respects:** Soft delete settings

- **`get${ModelName}ByMQuery(mQuery)`** — Retrieves a single record matching an MScript Query
  - **Parameters:** `mQuery` (Object) — MScript Query object (database-agnostic format)
  - **Returns:** Object|null — First matching record (ordered by createdAt DESC) or null
  - **Respects:** Soft delete settings

- **`get${ModelName}StatsByMQuery(mQuery, stats)`** — Calculates statistics on matching records
  - **Parameters:**
    - `mQuery` (Object) — MScript Query object (database-agnostic format)
  ...
  - **Example:** `await DB.getOrderStatsByMQuery({ status: "paid" }, ["count", "sum(totalAmount)"])` returns `{ count: 10, "sum-totalAmount": 5000 }`

- **`get${ModelName}ListByQuery(query)`** *(deprecated)* — Use `get${ModelName}ListByMQuery` instead
  - **Parameters:** `query` (Object) — Sequelize or MongoDB query object (depending on your DB type)

- **`get${ModelName}ByQuery(query)`** *(deprecated)* — Use `get${ModelName}ByMQuery` instead
  - **Parameters:** `query` (Object) — Sequelize or MongoDB query object

- **`get${ModelName}StatsByQuery(query, stats)`** *(deprecated)* — Use `get${ModelName}StatsByMQuery` instead
  - **Parameters:** `query` (Object) — Sequelize or MongoDB query object, `stats` (Array|String)
...
- **`update${ModelName}ByMQuery(dataClause, mQuery, context)`** — Updates all records matching an MScript Query
  - **Parameters:**
    - `dataClause` (Object) — Fields to update
    - `mQuery` (Object) — MScript Query object (database-agnostic format, see [MScript Query Operators Reference](./mscript-query-reference))
    - `context` (Object, optional) — Context object with `session` and `requestId`. When called from Business API actions, pass `this`.
  - **Returns:** Array — Array of updated record data
  - **Auto-handles:** Elasticsearch indexing for all updated records

- **`update${ModelName}ByQuery(query, dataClause, context)`** *(deprecated)* — Use `update${ModelName}ByMQuery` instead
  - **Parameters:** `query` (Object) — Sequelize or MongoDB query object, `dataClause` (Object), `context` (Object, optional)

Array field mutation in update functions is supported with `dataClause` operators (`$add`, `$rem`, `$addIf`). See [Array Mutation Syntax Guide](./array-mutation-syntax-guide).
...
- **`delete${ModelName}ByMQuery(mQuery, context)`** — Deletes all records matching an MScript Query
  - **Parameters:**
    - `mQuery` (Object) — MScript Query object (database-agnostic format, see [MScript Query Operators Reference](./mscript-query-reference))
    - `context` (Object, optional) — Context object with `session` and `requestId`. When called from Business API actions, pass `this`.
...
- **`delete${ModelName}ByQuery(query, context)`** *(deprecated)* — Use `delete${ModelName}ByMQuery` instead
  - **Parameters:** `query` (Object) — Sequelize or MongoDB query object, `context` (Object, optional)

#### **Query Format**

The `ByMQuery` functions accept the unified **MScript Query format**, which works identically regardless of the underlying database type:
...
See the [MScript Query Operators Reference](./mscript-query-reference) for the full list of supported operators.

The deprecated `ByQuery` functions accept native Sequelize or MongoDB query format:
...
```js
const { getOrderAggById, updateOrderById, getItemListByMQuery, getOrderStatsByMQuery } = require("dbLayer");
...
module.exports = async function processOrder(orderId, context = null) {
  ...
  // Get related items using MScript Query (database-agnostic)
  const items = await getItemListByMQuery({ orderId: orderId });

  // Calculate total using MScript Query
