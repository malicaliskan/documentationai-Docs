...
- **Compute & transform**
  `functionCallActions`, `addToContextActions`, `createObjectActions`, `renderDataActions`, `dataToFileActions`

- **CRUD orchestration**
  `createCrudActions`, `updateCrudActions`, `deleteCrudActions`

- **Service / API / Integrations**
  `interserviceCallActions`, `apiCallActions`, `integrationActions`, `paymentActions`
...
- **`onActionError`** *(enum:&#x20;*``*&#x20;|&#x20;*``*)* → error policy. `throwError` (default) aborts the API. `completeApi` catches the error, stores it as `this.<actionName>Error` on the context, and continues execution.
...
- Default **`onActionError: throwError`**; use `completeApi` only for non-critical enrichment. When `completeApi` is used, the error is stored as `this.<actionName>Error` so subsequent actions can inspect it.
...
**Array field updates:** When update payloads target array fields, prefer mutation operators (`$add`, `$rem`, `$addIf`) in `dataClause`. See [Array Mutation Syntax Guide](./array-mutation-syntax-guide).
...
For callback-success flows, keep in mind that payment processing may trigger a dedicated **payment-done** phase in generated assets (`paymentDone` event / post-callback hooks).  
Use idempotent post-payment logic for any operation that may be retried by provider callbacks.
...
### **21. ObjectPermissionCheckAction**
...
**Notes:** Keep enrichment failures non-fatal with `onActionError: "completeApi"`. The error is available as `this.<actionName>Error` in later actions.
