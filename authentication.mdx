...
- The `username` field accepts the user's primary login identifier: email when `primaryLoginIdentifier` is `'email'`, mobile when `'mobile'`, or either when `'emailOrMobile'`.
...
- Accepts either `username` or `email` fields (if both exist, `username` is prioritized). When `primaryLoginIdentifier` is `'mobile'`, pass the mobile number in E.164 format as `username`.
