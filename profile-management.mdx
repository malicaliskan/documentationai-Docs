...
**mobile** : A string value to represent the user's mobile number in E.164 format (e.g. `+905551234567`). The backend normalizes mobile numbers to this format automatically. This field is only available when the mobile field exists in the user data model (depends on `primaryLoginIdentifier` and `secondaryIdentifierPresence`). When mobile is the primary login identifier, it cannot be updated through the profile.
