...
| `onActionError` | String | Error handling strategy: `"throwError"` (default, aborts the API) or `"completeApi"` (catches the error, stores it as `this.<actionName>Error`, continues). |
...
- **Error handling matters**: Set `onActionError` on your IntegrationAction to control what happens when an external call fails (`"throwError"` for critical operations, `"completeApi"` for optional ones — the error is stored as `this.<actionName>Error`).
