...
Use in `ValidationAction.validationScript`, `ExtendedClause.condition`, or `BusinessApiAction.condition`:
...
Or as an `ExtendedClause.condition` to conditionally add filters:

```json
{
  "condition": "!!this.session.tenantId",
...
## 5. Cookbook: Server-computed / Derived Fields via `dataClauseItems`

Per-API write declarations live on each create / update BusinessApi as
`dataClauseItems[]`. Each entry has a `name` (the property to write)
and a `value` (an MScript expression — a literal, a property
reference, or an IIFE for multi-step logic). System-managed
properties (those whose value is never caller-supplied) carry only
DB-shape on the property declaration; the per-API entry is the
source of truth.

### 5.1 Simple Derived Field

`lineTotal = unitPrice * quantity`. On the property — DB-shape only:

```json
{
  "basicSettings": { "name": "lineTotal", "type": "Double", "isRequired": true }
}
```
...
{
  "dataClauseItems": [
    { "name": "lineTotal", "value": "this.unitPrice * this.quantity" }
  ]
}
```

`unitPrice` and `quantity` arrive via the API's `requestParameters[]`
(caller-supplied), and the derived `lineTotal` is computed at write
time without exposing it in the API contract.
...
{
  "name": "netAmount",
  "value": "(() => { const net = this.grossAmount - this.discountAmount; return Math.max(net, 0); })()"
}
```

This uses the inline arrow function trick for multi-step logic.
...
`dataClauseItems` entry:

```json
{ "name": "vatAmount", "value": "LIB.calcVat(this.netAmount, this.vatRate)" }
```

### 5.4 Different Value on Create vs Update

When the value differs across APIs, the per-API form already
expresses it directly — two different APIs, two different
`dataClauseItems` entries:

```json
// createOrder API
{ "name": "totalPoints", "value": "this.purchasePoints + (this.bonusPoints || 0)" }

// updateOrderPoints API
{ "name": "totalPoints", "value": "this.purchasePoints + (this.bonusPoints || 0) + (this.extraPoints || 0)" }
```

Per-API variation is the norm, not the exception — declaring write
behavior on the BusinessApi keeps each API's contract local and
explicit.
