---
title: "Auth Basics"
description: "Defines the fundamental authentication configuration for the project. These settings determine the authentication method(s) used across all services, including token-based, SSO, remote service, or API"
---
...
| Field                | Description                                                                                                                                                                                                                                                               |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| documentation        | Module-level UX prompt + freeform description of the auth service. Read by MindUI for login / register / verification / password-reset page generation.                                                                                                                   |
...
Holds the module-level frontendDocument UX prompt. Reused across auth / bff / notification modules for consistency. MindUI reads frontendDocument to generate page UX; the section exists so module-level primitives stay in proper sections rather than at module/chapter root.
...
| Field            | Description                                                                                                                                                                                           |
| ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| frontendDocument | Module-level UX prompt for MindUI. Describes intended page placement, navigation patterns, empty/error states, and any role-specific landing rules. Empty = MindUI works from technical schema alone. |
...
| Field                | Description                                                                                                                                                                                                                                                     |
| -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| apiKeyType           | The type of API keys to generate. 'secret' (default) creates sk*mbx*-prefixed keys that authenticate on behalf of a user and create server-side sessions. 'public' and 'both' options are reserved for future support — only 'secret' is currently implemented. |
| secretApiKeyLocation | Where secret API keys are sent in HTTP requests. Defaults to 'bearer' (Authorization: Bearer sk*mbx*...). When set to 'header' or 'query', the secretApiKeyName field specifies the header or query parameter name.                                             |
...
| secretApiKeyName     | Custom header or query parameter name for secret API keys. Only required when secretApiKeyLocation is not 'bearer'. When location is 'bearer', the sk*mbx* prefix is used for automatic detection and this field is ignored.                                    |
