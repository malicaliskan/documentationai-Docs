...
Mobile verification is available when the mobile field is present in the user data model (i.e., when `primaryLoginIdentifier` is `'mobile'` or `'emailOrMobile'`, or when `secondaryIdentifierPresence` is `'optional'` or `'required'` with email as primary).
