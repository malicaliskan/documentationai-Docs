...
>
> Older generated projects may expose checkout and campaign/coupon business APIs with `/v1/...` style routes. Newer projects can use different generated route conventions based on API naming and service generation templates. Always confirm exact paths from your generated service API documentation.

### **Checkout API**

The checkout API converts a cart into an order through a multi-step workflow.

**REST**: `POST /{generatedCheckoutRoute}/:id`

```javascript
const response = await fetch(`${serviceUrl}/${generatedCheckoutRoute}/${cartId}`, {
  method: "POST",
  headers: {
    "Content-Type": "application/json",
    "Authorization": `Bearer ${accessToken}`
...
|-----|--------|------|-------------|
| Create Campaign | POST | `/{generatedCreateCampaignRoute}` | Create a new campaign |
| Get Campaign | GET | `/{generatedGetCampaignRoute}/:id` | Get campaign by ID |
| List Campaigns | GET | `/{generatedListCampaignsRoute}` | List all campaigns |
| List Active | GET | `/{cartObjectName}campaigns/active` | List active campaigns (public) |
| Update Campaign | PATCH | `/{generatedUpdateCampaignRoute}/:id` | Update a campaign |
| Delete Campaign | DELETE | `/{generatedDeleteCampaignRoute}/:id` | Delete a campaign |

**Create Campaign Example:**

```javascript
const response = await fetch(`${serviceUrl}/${generatedCreateCampaignRoute}`, {
  method: "POST",
  headers: {
    "Content-Type": "application/json",
...
| API | Method | Path | Description |
|-----|--------|------|-------------|
| Create Coupon | POST | `/{generatedCreateCouponRoute}` | Create a new coupon |
| Get Coupon | GET | `/{generatedGetCouponRoute}/:id` | Get coupon by ID |
...
| List Coupons | GET | `/{generatedListCouponsRoute}` | List all coupons |
| Update Coupon | PATCH | `/{generatedUpdateCouponRoute}/:id` | Update a coupon |
| Delete Coupon | DELETE | `/{generatedDeleteCouponRoute}/:id` | Delete a coupon |
...
class CartService {
  ...
  async checkout(checkoutData) {
    ...
    const response = await fetch(
      `${this.apiBaseUrl}/${generatedCheckoutRoute}/${cart.id}`,
      {
        method: 'POST',
        headers: {
          ...this.getAuthHeaders(),
...
async function completeCheckoutWithPayment(checkoutData, paymentMethodId) {
  ...
  const checkoutResponse = await fetch(
    `${serviceUrl}/${generatedCheckoutRoute}/${cartId}`,
    {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${accessToken}`,
  ...
  const paymentResponse = await fetch(
    `${serviceUrl}/${generatedStartPaymentRoute}/${checkoutResult.orderId}`,
    {
      method: 'PATCH',
      headers: {
        'Authorization': `Bearer ${accessToken}`,
