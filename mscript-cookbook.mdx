...
## 5. Cookbook: Server-computed / Derived Fields (`source: 'customData'`)

Property-level formulas have been folded into the per-API customData mechanism. Set the property's `basicSettings.source` to `'customData'` and add a matching entry to every create/update API on the parent data object via `apiOptions.dataClauseSettings.customData[]`. The entry's `value` is an MScript expression — a literal, a property reference, or an IIFE for multi-step logic.

### 5.1 Simple Derived Field

`lineTotal = unitPrice * quantity`. On the property:

```json
{
  "basicSettings": { "name": "lineTotal", "type": "Double", "source": "customData" }
}
```
...
When the value differs across APIs, the per-API customData form already expresses it directly — no separate `updateFormula`, just two different APIs:
...
This is the main reason `source: 'customData'` replaced the old property-wide formula: per-API variation is the norm, not the exception.
