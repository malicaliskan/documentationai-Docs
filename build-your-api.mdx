...
> designer's explicit declaration. The explicit-mode quick reference below covers the
> 80% case. The legacy `autoParams` / `customParameters` / `customData` machinery is
> still valid for beta projects where `projectSettings.basicSettings.explicitMode` is
> false or absent — full reference further down this page.
...
- `dataClause` — used in **create** and **update** APIs.
  For array-field updates in `dataClause`, see [Array Mutation Syntax Guide](./array-mutation-syntax-guide).
...
> Everything below this divider describes Mindbricks' **implicit-mode** API design
> — the pattern shape used by beta projects created before the Q2 2026
> explicit-mode shift, identifiable by `projectSettings.basicSettings.explicitMode`
> being `false` or absent in the project config. New projects do NOT use these
> mechanics; the explicit-mode quick reference at the top of this page covers them.
>
> The implicit-mode machinery is preserved for back-compat with existing
> deployments. Reading this section is necessary only when working on a beta
> project; new project design should ignore everything below and stay with
> `requestParameters` + `dataClauseItems`.
...
When a Data Property has `basicSettings.source: 'session'`, its value is fetched from the session using the `sessionSettings.sessionParam` field. For example, a property like `userId` can automatically take its value from the logged-in user's session data during record creation.

Unlike `customData`-sourced or `joined`-sourced properties, **session parameters are still considered API parameters**, because their values are initialized **at the very beginning** of the API execution.
...
In contrast, **`customData`-sourced and `joined`-sourced properties** are evaluated **later**, right after the data clause is built and just before the database operation.
...
By default, `get`, `update`, and `delete` APIs use the **ID parameter** (located in the URL path) as the selection criterion. This ID parameter is automatically added to the API parameters **only as a fallback** — that is, when no `selectBy` or `fullWhereClause` is configured and the route path does not specify a different parameter.

The **`selectBy`** property of the `whereClause` defines which fields are used to identify a specific record.
When `selectBy` specifies a non-ID field (e.g., `["slug"]`), Mindbricks:

- **Does not** create the ID parameter.
- Builds the default route path using the `selectBy` field instead (e.g., `/:slug` instead of `/:productId`).
- Automatically generates a **select parameter** for each `selectBy` field.

This applies uniformly to **get**, **update**, **delete**, and **list** APIs.
...
- **Typical usage:**
  When no `selectBy` is configured, the default behavior is to use the ID parameter:
  ...
  When `selectBy` is set to a non-ID field, the route and parameter change accordingly:

  ```json
  "selectBy": ["email"]
  ```

  This produces `GET /users/:email` — the `email` parameter replaces the ID parameter entirely.

- **Parameter locations:**
  Select parameters are placed in `urlpath` when the route path contains them, or in `query` otherwise. When the default route is auto-generated from a single-field `selectBy`, that field is placed in the URL path. For multi-field `selectBy`, the ID parameter is used in the URL path and additional fields default to query parameters.
  If you manually design a route path and include parameter names in the URL, Mindbricks automatically expects those parameters in the `urlpath` as well.
...
- Automatically adds all `request`-sourced properties to the create payload.

- `session`-sourced and `joined`-sourced properties are excluded from the input parameter list (the runtime fills them) and inlined into the data clause directly.

- Custom data entries **override** any automatically generated fields. For properties whose `basicSettings.source` is `customData`, the customData entry IS the value source — every create API on this Data Object must declare one.
...
- Forced-on-create fields (e.g., default statuses like `"started"`) are expressed via `basicSettings.source: 'customData'` on the property, with each create API providing the literal in `apiOptions.dataClauseSettings.customData[]`.
...
For all API types except `list`, the where clause is expected to target **a single record**. For `update` and `delete`, the where clause is used to fetch the target instance, and the actual database mutation is performed using the fetched instance's ID to guarantee single-record safety.
...
| **topPick**           | Optional cap for ranked results. If `null` or `0`, no cap is applied. If set (e.g. `1000`), only the first N sorted records are eligible for pagination. |
...
Use `topPick` when your list is ranking-based and you want to paginate only within the first ranked items.

```json
"listOptions": {
  "listSortBy": [{ "property": "updatedAt", "order": "desc" }],
  "topPick": 1000
}
```
...
- `topPick = 1000` and `pageRowCount = 25` means users can browse pages only within the first 1000 sorted records.
- `topPick = 1` with descending activity/order sorting is useful for “latest active basket/order” style reads.
- `topPick = 0` or `null` keeps standard list behavior (no cap).
