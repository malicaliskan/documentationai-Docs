...
For array field updates using mutation operators (`$add`, `$rem`, `$addIf`) in `dataClause`, see [Array Mutation Syntax Guide](./array-mutation-syntax-guide).
...
- Supports array mutation syntax in `dataClause` via `$add`, `$rem`, `$addIf` (see [Array Mutation Syntax Guide](./array-mutation-syntax-guide))
...
### MScript Query Format (Recommended)

The `ByMQuery` functions accept the unified MScript Query format, which works identically regardless of the underlying database type. See the [MScript Query Operators Reference](./mscript-query-reference) for the full list of supported operators.
...
| `get${ModelName}ListByMQuery` | Get multiple by MScript Query | Array |
| `get${ModelName}ByMQuery` | Get single by MScript Query | Object\|null |
| `get${ModelName}StatsByMQuery` | Calculate statistics by MScript Query | Number\|Object |
...
| `update${ModelName}ByMQuery` | Update by MScript Query | Array |
| `delete${ModelName}ById` | Delete by ID | Object |
| `delete${ModelName}ByMQuery` | Delete by MScript Query | Array |
| `get${ModelName}ListByQuery` | *(deprecated)* Get multiple by native query | Array |
| `get${ModelName}ByQuery` | *(deprecated)* Get single by native query | Object\|null |
| `get${ModelName}StatsByQuery` | *(deprecated)* Calculate statistics by native query | Number\|Object |
| `update${ModelName}ByQuery` | *(deprecated)* Update by native query | Array |
| `delete${ModelName}ByQuery` | *(deprecated)* Delete by native query | Array |

All functions automatically handle Elasticsearch indexing, Kafka event publishing, and soft delete logic. Always prefer the `ByMQuery` variants for new code — they use the database-agnostic [MScript Query format](./mscript-query-reference) and work identically across all supported database types.
