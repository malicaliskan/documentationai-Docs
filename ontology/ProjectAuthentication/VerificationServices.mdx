...
| passwordResetByMobile | Configures SMS-based password reset functionality. Requires the mobile field to be present in the user data model (based on `primaryLoginIdentifier` and `secondaryIdentifierPresence`). |
| email2Factor          | Enables and configures two-factor authentication via email. Typically used during login or sensitive actions.                 |
| mobile2Factor         | Enables and configures two-factor authentication via SMS. Requires the mobile field to be present in the user data model (based on `primaryLoginIdentifier` and `secondaryIdentifierPresence`). |
