...
| `membershipCondition` | MScript | null | Optional MScript expression evaluated after fetching the membership record. Uses `membership` as the context variable. If it returns `false`, the user is denied access even though a record exists. Example: `membership.status == "active" && membership.expireAt > new Date()`. |
| `customAuthFunction` | String | null | Library function name for `custom` rule. |

### Cross-Service Membership

The membership check uses **Elasticsearch** rather than a direct database query. This allows the membership DataObject to live in a **different service** -- as long as it's indexed in Elasticsearch (which is the default for all DataObjects), the hub can query it across service boundaries. The Elasticsearch index name is automatically derived from the project codename and the membership DataObject name.
