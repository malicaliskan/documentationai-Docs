...
## Route Compatibility Guidance

Mindbricks API routes are generated from service/API definitions and generation templates.

- Older generated projects may use `/v1/...` style paths.
- Newer projects may expose normalized non-`/v1` routes.

For frontend stability:

- Keep route definitions centralized in one API client layer.
- Prefer reading generated API docs/assets after each regeneration.
- Treat hardcoded route literals in UI code as temporary scaffolding only.
