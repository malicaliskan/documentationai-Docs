...
| realtimeConfig | Configures the Socket.IO server for realtime hubs. Only relevant when the service has one or more RealtimeHub definitions. Controls the scaling adapter (Redis for multi-instance, memory for dev), heartbeat timings, and per-user connection limits. |
