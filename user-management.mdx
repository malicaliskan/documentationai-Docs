...
## User Data Model Field Availability

The presence of `email`, `mobile`, `emailVerified`, and `mobileVerified` fields in user objects depends on the authentication configuration:

- **`email` and `emailVerified`**: Present when `primaryLoginIdentifier` is `'email'` or `'emailOrMobile'`, or when `primaryLoginIdentifier` is `'mobile'` and `secondaryIdentifierPresence` is `'optional'` or `'required'`.
- **`mobile` and `mobileVerified`**: Present when `primaryLoginIdentifier` is `'mobile'` or `'emailOrMobile'`, or when `primaryLoginIdentifier` is `'email'` and `secondaryIdentifierPresence` is `'optional'` or `'required'`.

All user listing, creation, editing, and search operations in the admin dashboard should adapt to show only the fields that exist in the user data model.
...
The user management console in the admin dashboard should provide UX components for user creating by admins. When creating users, it should also be possible to upload user avatar. Note that when creating or updating users, admins cannot set `emailVerified` (or `mobileVerified` if the mobile field exists) as true, since verification is a logical mechanism and should be completed only through verification processes.
...
**email** : A string value to represent the user's email. Required when the email field exists in the user data model (depends on `primaryLoginIdentifier` and `secondaryIdentifierPresence`).
...
**mobile** : A string value to represent the user's mobile number in E.164 format (e.g. `+905551234567`). The backend normalizes mobile numbers to this format automatically. Required when the mobile field exists in the user data model (depends on `primaryLoginIdentifier` and `secondaryIdentifierPresence`).
...
User avatars are uploaded using the auth service's **database bucket (dbBucket)**. Both regular users (from the profile page) and admins (from the user management panel) upload avatars the same way:

`POST {authBaseUrl}/bucket/userAvatars/upload`

- **Authentication:** Regular access token in the Bearer header (no bucket token needed)
- **Content-Type:** `multipart/form-data`
- **Body:** the image file in the `files` field

The response returns an `accessKey`. Build the public download URL as:

`GET {authBaseUrl}/bucket/userAvatars/download/key/{accessKey}`

This URL is public (no auth required). Store it as the user's `avatar` property.

> **Note:** The external bucket service and `applicationBucketToken` are still used for non-avatar file uploads (product images, documents, etc.) to the application public bucket (`"tickatme-public-common-bucket"`).
...
**mobile** : A string value to represent the user's mobile number in E.164 format (e.g. `+905551234567`).
