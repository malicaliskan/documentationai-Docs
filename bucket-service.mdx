...
For small files like avatars, icons, and secret documents, Mindbricks also supports **database buckets** that store files directly in the service's database — no external bucket service needed. Database buckets use the regular access token instead of bucket-specific tokens.

The auth service automatically includes database buckets for user and tenant avatars. Any service can also define custom database buckets for its own file storage needs.

For full documentation on database buckets — configuration, auto-generated APIs, authorization, owner pairing, and examples — see the dedicated **[Database Buckets (dbBuckets)](/database-buckets)** guide.
