...
Customizes the REST endpoint path for a Business API. If omitted or set to `$default`, Mindbricks auto-generates the path based on the API name, crudType, and selector configuration. For single-record APIs (`get`, `update`, `delete`), the default path includes `/:objectId` only as a fallback; when `selectBy` specifies a non-ID field, the path uses that field instead (e.g., `/:slug`).
...
| Field           | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| routePath       | Custom REST path override. Leave as `$default` to auto-generate from the API name and selector. Auto-generated paths use `/:objectId` only when no `selectBy` or `fullWhereClause` is configured; otherwise, a single-field `selectBy` like `slug` produces `/:slug`. If you provide a custom path with a non-ID parameter (e.g., `/orders/:invoiceNbr`) and no `selectBy` is set, the parameter is automatically inferred as `selectBy`. The ID parameter is not created when the route does not contain `/:objectId`. If a custom path contains parameters that conflict with the configured `selectBy`, an ambiguity warning is raised during generation. |
