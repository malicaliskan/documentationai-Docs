...
- **`create${ModelName}(data)`** — Creates a single record
  - **Parameters:** `data` (Object) — The data object to create. If `id` is provided and exists, it will update the existing record instead.
...
- **`createBulk${ModelName}(dataList)`** — Creates multiple records in bulk
  - **Parameters:** `dataList` (Array) — Array of data objects to create
  - **Returns:** Array — Array of created/updated record data
  - **Auto-handles:** Same as single create, processes each item sequentially
...
- **`get${ModelName}AggById(id)`** — Retrieves a record with aggregated/joined data
  - **Parameters:** `id` (String|Array) — Single ID or array of IDs
  - **Returns:** Object|Array|null — Record(s) with joined relations populated via CQRS joins
...
- **`update${ModelName}ById(id, dataClause)`** — Updates a record by ID
  - **Parameters:**
...
- **`update${ModelName}ByIdList(idList, dataClause)`** — Updates multiple records by ID list
  - **Parameters:**
...
- **`update${ModelName}ByQuery(query, dataClause)`** — Updates all records matching a query
  - **Parameters:**
    - `query` (Object) — Sequelize or MongoDB query object
...
- **`delete${ModelName}ById(id)`** — Deletes a record by ID
  - **Parameters:** `id` (String|Object) — Record ID or object with `id` property
  - **Returns:** Object — Deleted record data
  - **Auto-handles:** Soft delete (if enabled), Elasticsearch deletion, Kafka events

- **`delete${ModelName}ByQuery(query)`** — Deletes all records matching a query
  - **Parameters:** `query` (Object) — Sequelize or MongoDB query object
...
module.exports = async function processOrder(orderId) {
  // Get order with aggregated data
  const order = await getOrderAggById(orderId);
  if (!order) throw new Error("Order not found");
  
  // Update order status
  await updateOrderById(orderId, { status: "processed" });
