---
title: "Business Api"
description: "Defines a Business API, a high-level abstraction representing a specific business operation on a primary data object. It encapsulates intent (e.g., get, create, delete), parameters, trigger mechanisms"
---
...
## Field applicability by `apiOptions.crudType`

Most option blocks apply to every crudType, but the data-shaping clauses and the `*Options` blocks are restricted. Populating a block on a crudType that ignores it is a design-time mistake: the codegen silently drops the configuration. The validator emits a `BUSINESSAPI_BLOCK_IGNORED` warning when this happens.

| Block               | create | get | list | update | delete |
| ------------------- | :----: | :-: | :--: | :----: | :----: |
| `apiOptions`        |   ✓    |  ✓  |  ✓   |   ✓    |   ✓    |
| `authOptions`       |   ✓    |  ✓  |  ✓   |   ✓    |   ✓    |
| `requestParameters` |   ✓    |  ✓  |  ✓   |   ✓    |   ✓    |
...
- `selectClause` shapes the read projection — only meaningful when the API actually returns records (`get`, `list`).
- `whereClause` locates the target record(s). It applies to every read or mutation that has to identify what to operate on; `create` is excluded because it makes a new record.
- `dataClauseItems` carries the write payload — each entry maps a property name to a value MScript or literal that the framework writes at create / update time. `sessionSettings` and `staticJoin` properties are auto-injected and don't need entries here. Only `create` and `update` mutate fields; `delete` mutates only the soft-delete flag (configured via `deleteOptions`).
- `getOptions` / `listOptions` / `paginationOptions` / `deleteOptions` are the four crudType-specific knobs. Each is meaningful for exactly one crudType.

When in doubt, consult the per-field doc on each option block — every block whose applicability is restricted repeats the rule in its own `__doc`.
...
| requestParameters | The full request shape of this API — every parameter the caller can supply, declared inline. The framework binds `this.<name>` from the request based ONLY on this list. Element shape: name, type, isArray, required, httpLocation, defaultValue, transform, description, isSecret, hidden. URL placeholders in `restSettings.routePath` (e.g. `:teamId`) auto-promote into this list with `httpLocation: 'urlpath'` if not declared explicitly.                                                                                                                                                                                          |
| dataClauseItems   | Lists every dataClause field the framework writes at create / update time, with the value MScript that produces it. Top-level on the BusinessApi. Runtime composition: for each property on the data object, (1) `sessionSettings` block → auto-inject from session, (2) `staticJoin` block → run the join, expose result via `contextParameterName`, (3) otherwise → look up matching `dataClauseItems[i].name` and evaluate its `value`. Properties with none of the three are not written (auditor flags missing entries). Common value shapes: `this.<paramName>` (caller-supplied), `'<literal>'` (system-managed), `this.<paramName> |     | '<default>'`(caller-with-fallback), MScript like`LIB.calc(this.X)` for computed. |
...
| whereClause       | Defines the criteria used to locate the target record(s) for the main operation. This is expressed as a query object and applies to `get`, `list`, `update`, and `delete` APIs. All API types except `list` are expected to target a single record. For `update` and `delete`, the WHERE clause is used to fetch the target instance, and the actual database mutation is performed using the fetched instance's ID to guarantee single-record safety.                                                                                                                                                                                     |
...
| listOptions       | Defines list-specific options including filtering logic, default sorting, and result customization for APIs that return multiple records. Applies only to `list` type APIs; ignored on every other crudType.                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| testingNotes        | Free-text markdown block written by the designer at apply time, describing how this API's happy path should be tested. Read VERBATIM by the api-test-plan blueprint as the AUTHORITATIVE happy-case source — overrides the planner's role/body-field inference rule when present. Write down only what the planner cannot statically infer from authOptions + the data-object schema + the route doc: custom validateAction rules (e.g. 'rejects 403 when seller.isActive=false'), sequencing prereqs ('must be called after checkout'), side effects worth asserting (kafka events, related-record writes), auto-populated session fields (sellerId, ownerId, tenantId — keep these out of the request body), conditional status codes (200 vs 201 vs 409). Format: short markdown following the template in the apply prompt — Happy path / Side effects / Custom rules / Skip cases. Empty is allowed (planner falls back to inference) but the validator will warn. |
...
Defines the REST controller configuration for a Business API. REST is the default HTTP interface in Mindbricks. These settings allow enabling/disabling REST access and customizing the endpoint path. Fields are flat — set `routePath` and `forcePOSTMethod` directly on this block (the legacy `configuration: { ... }` wrapper is still accepted on input but lifted at validate time).
...
| routePath         | Custom REST path override. Leave as `$default` to auto-generate.                                      |

## Default-route calculation

Given the API's crudType, name, dataObjectName, and whereClause.selectBy, the calculator produces:

create / list → /&lt;resourcePath>
get / update / delete → /&lt;resourcePath>/:&lt;param> (always gets a path param)

The `<param>` segment is chosen in this order:

1. If `whereClause.selectBy` has exactly one entry and that entry is NOT `id`,
   the param uses that field name. Example: selectBy=['slug'] → /&lt;resource>/:slug.
2. Otherwise the param is the data-object's natural id, formatted as `<dataObjectName>Id`.
   Example: dataObject=`User` → /&lt;resource>/:userId.

`<resourcePath>` is derived from the API name + dataObjectName, ALWAYS lowercased:

- Default-shape APIs (e.g. getUser/listUsers/createUser/updateUser/deleteUser on the User
  object) collapse to the plural object name: `/users`.
- Non-default APIs (custom verbs like `archiveUser`, `closeOrder`) concatenate verb + singular
  object name (no plural): `/archiveuser`, `/closeorder`.

The path parameter follows the standard Mindbricks naming convention — `dataObject.objectName` is lowercase camelCase (e.g. `user`, `orderItem`), and the param appends `Id`: `:userId`, `:orderItemId`. (The resource segment is additionally lowercased; the param name passes the input case through.)

## fullWhereClause no longer removes the path parameter

Deliberate change from earlier behavior. Previously, setting `whereClause.fullWhereClause` on a get/update/delete API would drop the `/:id` segment and collapse the route onto the bare resource path — which collided with the list/create endpoint at `GET /<resource>`. The default calculator now keeps the path parameter regardless of fullWhereClause. If you intentionally want a parameter-less single-record route (verb-style endpoint that resolves the record from auth context, fullWhereClause, etc.), set `routePath` explicitly.

## Custom routePath rules

- Always start with `/`.
- If your custom path includes a parameter (`:something`) AND `selectBy` is unset, the parameter name is inferred as the selectBy field. Example: `routePath: '/orders/:invoiceNbr'` with no selectBy → effective selectBy is `invoiceNbr`.
- If your custom path's parameter name conflicts with the configured `selectBy`, an ambiguity warning is raised at validation.
- A custom routePath without any `:`-style parameter IS allowed for single-record get/update/delete APIs — useful for verb-style endpoints (e.g. `/getActiveBasket`, `/closeMyOrder`) where the record is resolved from auth context or `fullWhereClause` rather than the URL. Just be aware the route cannot vary per record, so it must not collide with the list/create path on the same resource.

## Verifying the calculated route

After writing your business APIs, the apply worker can call `getServiceBusinessApiList(serviceName)` to get the structured list of `{name, crudType, method, routePath, source}` for every API in the service — useful to confirm the auto-calculator produced the route shape you expected before triggering a build.
forcePOSTMethod | The http method will be set according to the crud type, use this setting if you want to force the HttpMethod to POST. Not recommended if you don't have a specific reason.

## ApiGrpcSettings

**MPO Version:** 1.3.0

Enables gRPC interface for the Business API. Useful for high-performance service-to-service communication. Fields are flat — set `responseFormat` and `responseType` directly (the legacy `configuration: { ... }` wrapper is still accepted on input but lifted at validate time).
...
Configures Kafka as a trigger mechanism for the Business API. This supports both asynchronous processing and integration-based event handling (e.g., storage triggers, audit flows). Fields are flat — set `requestTopicName` and `responseTopicName` directly (the legacy `configuration: { ... }` wrapper is still accepted on input but lifted at validate time).
...
Schedules the Business API for periodic execution using a cron expression. Ideal for background jobs or recurring tasks. Fields are flat — set `cronExpression` directly (the legacy `configuration: { ... }` wrapper is still accepted on input but lifted at validate time).
...
| Field             | Description                                                                                                     |
| ----------------- | --------------------------------------------------------------------------------------------------------------- |
| hasCronController | Enables or disables the cron controller. If true, the API will run at the interval defined by `cronExpression`. |
...
Defines how records are filtered in a Business API. This includes:- `selectBy`: strict selection logic based on required field equality. It also drives the default route path and parameter creation for single-record APIs (`get`, `update`, `delete`). When a non-ID field is set as `selectBy`, the route uses that field instead of the object ID, and the ID parameter is suppressed. - `fullWhereClause`: an optional override for complex, flexible query logic. When set, no ID parameter is auto-created and the route path defaults to the resource base path. - `additionalClauses`: optional conditional query fragments that are always appended to the WHERE clause, whether it is derived from `selectBy` or `fullWhereClause`. Use `selectBy` when your selection logic is static and business-critical (e.g., `['slug']`). Use `fullWhereClause` when the WHERE clause needs to be built dynamically using MScript. `additionalClauses` allow context-aware filtering such as role-based visibility or soft-delete exclusion. For `update` and `delete` operations, the actual database mutation always targets the single record fetched by `fetchInstance`, regardless of the WHERE clause breadth. Selector resolution order for `get`/`update`/`delete`: (1) explicit `fullWhereClause`; (2) explicit `selectBy`; (3) inferred `selectBy` from non-id `:` segments in the route path; (4) the standard `:{objectName}Id` (or `:id`) URL parameter; (5) latest-record fallback — when none of the above apply the pipeline sets `fullWhereClause = "{}"` and dbLayer's single-record fetcher returns the most recent record (ordered by `createdAt` DESC). The fallback is intentional support for current-context endpoints (`GET /snapshots/latest`, `DELETE /drafts/current`); when it triggers automatically, validate emits a `LATEST_RECORD_FALLBACK` warning so the designer can confirm intent. Suppress the warning by setting `fullWhereClause: "{}"` explicitly when latest-record semantics is desired.
...
| Field             | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| selectBy          | A list of fields that must be matched exactly as part of the WHERE clause. This is not a filter — it is a required selection rule. In single-record APIs (`get`, `update`, `delete`), it defines how a unique record is located. In `list` APIs, it scopes the results to only entries matching the given values. When a single non-ID field is specified (e.g., `['slug']`), the default route path is built as `/:slug` instead of `/:objectId`, and the ID parameter is not auto-created. If a custom route path contains non-ID parameters and no `selectBy` is configured, the route parameters are automatically inferred as `selectBy`. For the `id` field, both `/:objectId` and `/:id` are accepted as valid parameter sources. |
...
Defines an optional filtering clause that is dynamically included in the query when a single MScript condition is met. Enables route-level filtering to adapt to session, context, or other logic at runtime.
...
| condition   | An MScript expression that gates whether `whereClause` is merged into the query at runtime. The clause applies when the expression evaluates truthy. Leave null to apply the clause unconditionally. To express an exclusion (apply when X is NOT true), negate the expression yourself, e.g. `!this.session?.isAdmin`. |
| whereClause | The conditional query fragment to include, written as an MScript Query. The clause is merged into the final query when `condition` is truthy (or when `condition` is null, in which case the clause always applies).                                                                                                    |
...
interface BusinessApiActionStore = {
  ...
  agentCallActions : AgentCallAction[];
...
| agentCallActions            | Actions that invoke a configured AI Agent defined in the service's aiAgents section. The agent is executed internally with a data payload built from the current API context via MScript. The result is stored in the workflow context and optionally written to the response.                                                                             |
...
| integrationActions          | Actions that call third-party API integrations managed in the Mindbricks integration catalog (e.g., AWS S3, Stripe, Google Maps). Each integration exposes predefined actions and is selected by its provider key.                                                                                                                                         |
...
Abstract base class for all Business API actions. Every action in a Business API workflow inherits from this type. It defines shared fields such as name, execution condition, and response behavior. This class is never instantiated directly—subclasses implement specific logic units like fetch, validation, or API calls. Any subclass is distinguished from the array (in actions property of the business api) it is inserted

```typescript
interface BusinessApiAction = {
  id : String;
  name : String;
  description : Text;
  condition : MScript;
  ...
  onActionError : BusinessApiActionErrorOption;
  emitProgressInSSEContext : Boolean;
}
```

| Field                    | Description                                                                                                                                                                                                                                                                              |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| id                       | The unique identifier of the action, used to reference this action from within a Business API workflow step.                                                                                                                                                                             |
| name                     | A unique human readable camle case name for the action , will also be used as the method name of the api context class.                                                                                                                                                                  |
| description              | An optional text to describe the purpose and logic of the action, what it does and why it does.                                                                                                                                                                                          |
| condition                | An MScript boolean expression. If provided, the action will only run when this condition evaluates to true.                                                                                                                                                                              |
| contextPropertyName      | The name of the property under which the result of the action will be written to the context. The result of an business api action can be accessed by other actions using `this.<contextPropertyName>`. Leave this field null if your action result will not be used by further actions. |
| writeToResponse          | If true, the result of the action will be written to the API response directly, typically under the property matching the contextPropertyName.                                                                                                                                           |
| onActionError            | Defines what happens when the action throws an error. `throwError` (default) re-throws and aborts the API. `completeApi` catches the error, stores it as `this.<actionName>Error` on the context so subsequent actions can inspect the failure, and lets the API continue.               |
| emitProgressInSSEContext | When true and the API is running in SSE context, this action automatically emits a progress event before and after execution. When false (default) or when called via REST, no progress is emitted.                                                                                      |

#### BusinessApiActionErrorOption

The option that defines what to do if a specific action causes an error.

```js
const BusinessApiActionErrorOption = {
...
| Enum        | Description                                                                               |
| ----------- | ----------------------------------------------------------------------------------------- |
| throwError  | Use this option if you want to fail the compile api in case of this specific action fails |
| completeApi | Use this option if you want your api to be completed even this specific action fails.     |
...
| dataValue    | The MScript expression or static value used to assign a value to the specified property. This can reference current context (`this`), a parameter (`this.input`), or the loop iteration variable when executed inside a `LoopAction` — by the name configured on `LoopAction.loopItemName` (e.g. if `loopItemName` is `customerItem`, reference `customerItem.id`). The legacy name `crudItem` is also bound as an alias for backward compatibility with older specs. |
...
Invokes an AI Agent defined in the service's aiAgents section from within a Business API workflow. The agent is executed internally using a data payload constructed from the current API context via an MScript expression. The agent result is stored in the workflow context and can be used by subsequent actions or written to the response. This enables tight integration between structured API logic and AI agent capabilities.

```typescript
interface AgentCallAction extends BusinessApiAction = {
  agentName : String;
  inputData : MScript;
}
```
...
| Field     | Description                                                                                                                                                                                                                                                                                                                                                          |
| --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| agentName | The name of the AI Agent to invoke. Must match the `agentBasics.name` of an agent defined in the service's `aiAgents` array. Example: 'contentAnalyzer', 'summarizer'.                                                                                                                                                                                               |
| inputData | An MScript expression that builds the data object to pass to the agent as its request body. Has access to `this` (the API context), including `this.data`, `this.session`, and any context properties set by previous actions. Example: `{ message: this.data.description, context: this.fetchedProfile.bio }`. If null, the current `this.data` is forwarded as-is. |
...
Calls a third-party API integration from the Mindbricks integration catalog (e.g., AWS S3, Stripe, Google Maps). The provider is identified by its string key matching an integration picked for the project. Each integration exposes a set of predefined actions, and this object triggers one of them with dynamically computed parameters. The result is stored in the context and can be used in follow-up steps.
...
| Field      | Description                                                                                                                                                                                                             |
| ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| provider   | The provider key identifying the integration to call (e.g., `amazonS3`, `Stripe`, `googleMaps`). Must match the providerKey of an integration that has been picked for the current project.                             |
| action     | The specific action or method to invoke on the integration provider. For example: `uploadFile`, `createRecord`, `deleteObject`, `listItems`. Action names must match the methods exposed by the integration class code. |
...
Defines a repeated execution block within a Business API workflow. It loops over the result of an MScript expression and executes a list of actions for each item. The current item is exposed as a local variable named by `loopItemName`, directly accessible in MScript. In-actions executed under a loop receive a loop-step signature context (`loopItem`, `stepIndex`, `stepResults`) so step-scoped intermediate values can be referenced safely, especially in parallel loops. Loop actions can be nested to support multi-level iteration, enabling hierarchical or grid-like data processing patterns.
...
| isParallel   | A boolean value to control the non-blocking behaviour of the loop steps. If `isParallel` is true, all loop steps are scheduled asynchronously and awaited as a group. If false, steps are awaited and executed one by one. Warning: parallel mode can cause race conditions when actions write to shared context (`this.*`) or perform conflicting read-modify-write updates on the same record. Prefer item-local writes, `stepResults`, or database concurrency controls when enabling parallel mode. |
