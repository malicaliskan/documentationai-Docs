...
- `DataMapItem.value` — including every entry in `apiOptions.dataClauseSettings.customData[]` (the source for properties with `basicSettings.source: 'customData'`)
...
#### 2.4.2 Server-computed Field via `source: 'customData'`

A `totalPrice` property derived as `unitPrice * quantity`. On the property:

```json
{
  "basicSettings": { "name": "totalPrice", "type": "Double", "source": "customData" }
}
```

On every create/update API of the parent data object:
...
Then declare `shippingFee` as a `customData`-sourced property:
...
…and on each create/update API of the parent data object, add a customData entry that calls the library:
...
Best of both worlds: explicit per-API source + custom calculation logic.
